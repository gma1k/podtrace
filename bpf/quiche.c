...
static long quiche_hdr_cb(u32 idx, void *vctx)
{
	...
	if (bpf_probe_read_user(&h, sizeof(h), addr) != 0)
		return 1;
	if (!h.name || !h.value || h.name_len == 0 || h.name_len > H3_HDR_NAME_MAX)
		return 0;
	...
	u32 nlen = (u32)h.name_len;
	if (nlen == 0 || nlen > H3_HDR_NAME_MAX)
		return 0;
	asm volatile("" : "+r"(nlen));
	nlen &= (2 * H3_HDR_NAME_MAX) - 1;
	if (bpf_probe_read_user(nm, nlen, (void *)h.name) != 0)
		return 0;

	if (nlen == 7 && nm[0] == ':' && nm[1] == 'm' && nm[2] == 'e' &&
	    nm[3] == 't' && nm[4] == 'h' && nm[5] == 'o' && nm[6] == 'd') {
		u32 n = nghttp3_clamp_len(h.value_len, H3_TXN_METHOD_MAX);
		if (n && bpf_probe_read_user(rec->method, n, (void *)h.value) == 0)
			rec->method_len = (u8)n;
		return 0;
	}
	if (nlen == 5 && nm[0] == ':' && nm[1] == 'p' && nm[2] == 'a' &&
	    nm[3] == 't' && nm[4] == 'h') {
		u32 n = nghttp3_clamp_len(h.value_len, H3_TXN_PATH_MAX);
		if (n && bpf_probe_read_user(rec->path, n, (void *)h.value) == 0)
			rec->path_len = (u16)n;
		return 0;
	}
	if (nlen == 7 && nm[0] == ':' && nm[1] == 's' && nm[2] == 't' &&
	    nm[3] == 'a' && nm[4] == 't' && nm[5] == 'u' && nm[6] == 's') {
		rec->status = nghttp3_parse_status(h.value, h.value_len);
		return 0;
	}
	if (nlen == 11 && nm[0] == 't' && nm[1] == 'r' && nm[2] == 'a' &&
	    nm[3] == 'c' && nm[4] == 'e' && nm[5] == 'p' && nm[6] == 'a' &&
	    nm[7] == 'r' && nm[8] == 'e' && nm[9] == 'n' && nm[10] == 't') {
		u32 n = nghttp3_clamp_len(h.value_len, H3_TXN_TP_MAX);
		if (n && bpf_probe_read_user(rec->traceparent, n, (void *)h.value) == 0)
			rec->tp_len = (u8)n;
		return 0;
	}
	nghttp3_capture_hdr_rec(rec, nm, nlen, h.value, h.value_len);
	return 0;
}
